...
TEST_CASE("a tool jog window seeds its start pose from the published tool pose before anything is driven", "[manipulator][controls]")
{
    const std::shared_ptr<arm_publisher> published = publishing(chosen_snapshot());
    const std::shared_ptr<edited_pose> held        = std::make_shared<edited_pose>();
    tool_jog_window panel("Tool frame jog", published->reader(), std::weak_ptr<owned_arm>(), reference, held, {mode::preview});
...
TEST_CASE("a tool jog window over a publication carrying no tool pose seeds from the one that follows", "[manipulator][controls]")
{
